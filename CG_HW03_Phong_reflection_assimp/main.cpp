...
void key_callback(GLFWwindow * window, int key, int scancode, int action, int mods)
{
  ...
  if (key == GLFW_KEY_Q && action == GLFW_PRESS) camera.set_mode(camera.mode() == Camera::kOrtho ? Camera::kPerspective : Camera::kOrtho);
  if (key == GLFW_KEY_L && action == GLFW_PRESS) light_position_wc(0) -= 5;
  if (key == GLFW_KEY_R && action == GLFW_PRESS) light_position_wc(0) += 5; 
  if (key == GLFW_KEY_D && action == GLFW_PRESS) light_position_wc(1) -= 5;
  if (key == GLFW_KEY_U && action == GLFW_PRESS) light_position_wc(1) += 5;
  if (key == GLFW_KEY_B && action == GLFW_PRESS) light_position_wc(2) -= 5;
  if (key == GLFW_KEY_F && action == GLFW_PRESS) light_position_wc(2) += 5;
  if (key == GLFW_KEY_P && action == GLFW_PRESS) g_is_animation = !g_is_animation;
}
