...
void init_buffer_objects()
{
  ...
  for (int i = 0; i < scene->mNumMeshes; ++i){
    ...
    const aiMesh* mesh = scene->mMeshes[i];
    mesh_obj.material_index = mesh->mMaterialIndex;
    ...
    glGenBuffers(1, &mesh_obj.normal_buffer);
    glBindBuffer(GL_ARRAY_BUFFER, mesh_obj.normal_buffer);
    if (mesh->mNormals != NULL){
    ...
    for (int j = 0; j < scene->mNumMaterials; j++) {
        glGenBuffers(1, &mesh_obj.texcoord_buffer);
        glBindBuffer(GL_ARRAY_BUFFER, mesh_obj.texcoord_buffer);
        if (mesh->mTextureCoords[0] != NULL){
...
{   
  ...
  for (int i = 0; i < scene->mNumMaterials; i++) {
    ...
    for (unsigned int j = 0; j < diffuseTextureCount; j++) {
      aiString textureFilePath;
      if (AI_SUCCESS == scene->mMaterials[i]->GetTexture(aiTextureType_DIFFUSE, j, &textureFilePath)){
        std::string texfile = textureFilePath.data;
        base_path[0] = {0};
        strcpy(base_path, basepath.c_str());
        file_name = strcat(base_path, texfile.c_str());
      }
    }

    image = stbi_load(file_name, &width, &height, &channels, STBI_rgb);
    glGenTextures(1, &texture_map[file_name]);
    glBindTexture(GL_TEXTURE_2D, texture_map[file_name]);
...
void draw_mesh(const aiMesh* mesh, const aiMatrix4x4& mat_model)
{
  ...
  glUniform1i(loc_u_diffuse_texture, 1);
  glActiveTexture(GL_TEXTURE1);

  glUniform3fv(loc_u_view_position_wc, 1, (float*)&camera.mPosition);   // view position
  
  glUniform3f(loc_u_light_position_wc, light_position_wc.x, light_position_wc.y, light_position_wc.z);
  glUniform4f(loc_u_light_ambient, light_ambient.r, light_ambient.g, light_ambient.b, light_ambient.a);
  glUniform4f(loc_u_light_diffuse, light_diffuse.r, light_ambient.g, light_ambient.b, light_ambient.a);
  glUniform4f(loc_u_light_specular, light_specular.r, light_specular.g, light_specular.b, light_specular.a);

  glUniform4f(loc_u_material_ambient, material_ambient.r, material_ambient.g, material_ambient.b, material_ambient.a);
  glUniform4f(loc_u_material_specular, material_specular.r, material_specular.g, material_specular.b, material_specular.a);
  ...
  for (int i = 0; i < scene->mNumMaterials; i++) 
  {
    ...
    aiString textureFilePath;
    int texIndex = 0;
    if(AI_SUCCESS == scene->mMaterials[mesh->mMaterialIndex]->GetTexture(aiTextureType_DIFFUSE, texIndex, &textureFilePath)){
		  //bind texture
      std::string texfile = textureFilePath.data;
      base_path[0] = {0};
      strcpy(base_path, basepath.c_str());
      file_name = strcat(base_path, texfile.c_str());
	    unsigned int texId = texture_map[file_name];
  ...
  for (int i = 0; i < scene->mNumMeshes; i++) {
    const aiMesh* mesh;
    const kmuvcl::Mesh& mesh_obj = meshes[i];
    ...
    for (int j = 0; j < mesh_obj.faces.size(); ++j){
      const kmuvcl::Face& face = mesh_obj.faces[j];
